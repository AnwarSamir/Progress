...
public class AsiProgressWheel extends View {
  private static final String TAG = AsiProgressWheel.class.getSimpleName();
   ...
   */
  public AsiProgressWheel(Context context, AttributeSet attrs) {
    super(context, attrs);

    parseAttributes(context.obtainStyledAttributes(attrs, R.styleable.AsiProgressWheel));
   ...
   * The constructor for the ProgressWheel
   */
  public AsiProgressWheel(Context context) {
    super(context);
    setAnimationEnabled();
  }
  ...
  private void parseAttributes(TypedArray a) {
    ...
    circleRadius =
        (int) a.getDimension(R.styleable.AsiProgressWheel_matProg_circleRadius, circleRadius);

    fillRadius = a.getBoolean(R.styleable.AsiProgressWheel_matProg_fillRadius, false);

    barWidth = (int) a.getDimension(R.styleable.AsiProgressWheel_matProg_barWidth, barWidth);

    rimWidth = (int) a.getDimension(R.styleable.AsiProgressWheel_matProg_rimWidth, rimWidth);

    float baseSpinSpeed =
        a.getFloat(R.styleable.AsiProgressWheel_matProg_spinSpeed, spinSpeed / 360.0f);
    spinSpeed = baseSpinSpeed * 360;

    barSpinCycleTime =
        a.getInt(R.styleable.AsiProgressWheel_matProg_barSpinCycleTime, (int) barSpinCycleTime);

    barColor = a.getColor(R.styleable.AsiProgressWheel_matProg_barColor, barColor);

    rimColor = a.getColor(R.styleable.AsiProgressWheel_matProg_rimColor, rimColor);

    linearProgress = a.getBoolean(R.styleable.AsiProgressWheel_matProg_linearProgress, false);

    if (a.getBoolean(R.styleable.AsiProgressWheel_matProg_progressIndeterminate, false)) {
      spin();
    }
